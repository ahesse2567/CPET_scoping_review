---
title: "Popularity And Prevalence Of Gas Exchange Data Processing Methods: A Semi-Automated Scoping Review"
...
The importance of attaining a VO~2~ plateau to accurately determine VO~2~max has been highlighted because "secondary" VO~2~max criteria often underestimate VO~2~max [@poole2017], and thus misclassify cardiorespiratoroy fitness.
Finally, we are unaware of research on the effects of data processing and locating ventilatory thresholds.

Outlier removal is often accomplished by removing points beyond 2, 3, or 4 standard deviations (SD) beyond the local mean (i.e., a prediction interval) [@nolte2023].
...
Although this does not affect parameter estimates, one-second interpolation has been criticized for artificially narrowing confidence intervals as respiratory rates are usually below below 60 breaths per minute, even near maximal exercise [@benson2017; @francescato2014; @francescato2019; @francescato2015].
...
A more recent scoping review by Nolte et al. @nolte2023 on papers published between 2017 and March 2022 assessing VO~2~max with ramp protocols found that nearly half of their included studies did not report their data processing steps.
...
This research expands on that of @nolte2023 but searches without date restriction and includes studies with CPET data beyond VO~2~max ramp tests.
...
This review assesses the frequency of reporting, and popularity of outlier removal, interpolation, and data averaging methods.
The results emphasize the importance of stating these methods to improve reproducibility and documents the data processing choices worth testing in future research investigating the effects of such choices on CPET values.

# Methods {#sec-methods}

## Protocol and Registration

These methods [@tricco2018] and results [@peters2020] are modeled on the PRISMA scoping review extension guidelines and PRISMA 2020 statement [@page2021]. This work was first registered with the Open Science Framework [@foster2017; -@hesse2022] and is based on a dissertation chapter by the first author [-@hesse2023].
The code and most data for this project are available on GitHub.
...
Due to licensing restrictions and text and data mining agreements, we are unable to share the full-text files.
...
We acquired data from the Ovid-MEDLINE, Scopus, and Web of Science databases with the guidance of a university librarian on 2022-06-27.
The electronic search strategies for all databases can be found in the supplemental materials.
...
The correspobnding PRISMA flow diagram was created using the PRISMA2020 R package [@haddawayPRISMA2020PackageShiny2022].
...
Regular expressions are sequences of characters that searches for specific text.
A simple example of a RegEx is searching for specific text using cmd/ctrl+F to search for text in a document.
...
We manually reviewed every article flagged as ineligible by the ML classifier.
Our classifiers also calculated an eligibility probability.
We also manually reviewed articles within \~15% of the decision boundary.
We iteratively rebuilt our classifiers with more data as we manually confirmed article eligibility.
Through rebuilding our classifiers we discovered additional ineligible articles missed by previous classifier iterations.
...
Given the large number of articles, we needed a minimum sample size of `r format(n_articles, big.mark = ',', scientific = FALSE)` based on a 95% confidence interval and a maximum margin of error (MOE) of ±3%, assuming a proportion of 0.5 for a conservative estimate.
...
Counts, proportions, and their 95% confidence intervals using the Agresti-Coull method were calculated for the reporting frequency of each data processing method using R `r str_extract(R.version.string, "R version \\d\\.\\d\\.\\d")` [@rcoreteam2021] in the RStudio IDE version 2023.6.1.524 [@rcoreteam2021].
When articles reported multiple methods, we only counted this article once for calculating reporting proportions.
...
Our initial search discovered `r format(nrow(all_articles), big.mark = ',', scientific = FALSE)` articles.
We attempted to retrieve the remaining `r format(nrow(articles_unique), big.mark = ',', scientific = FALSE)` after removing duplicates and articles without a DOI.
A total of `r format(nrow(bbb_eligible), big.mark = ',', scientific = FALSE)` articles analyzed were included in the interpolation and outlier analysis.
...
Of the `r format(total_articles, big.mark = ',', scientific = FALSE)` articles, `r moe_prop_articles_reporting$x` (`r sprintf("%.1f", moe_prop_articles_reporting$mean * 100)`%, 95% CI: `r sprintf("%.1f", moe_prop_articles_reporting$lower * 100)`-`r sprintf("%.1f", moe_prop_articles_reporting$upper * 100)`%) reported outlier removal methods.
Of the articles reporting their outlier methods, the most prevalent methods were ±3 (`r sprintf("%.1f", prop_3sd * 100)`%) and ±4 (`r sprintf("%.1f", prop_4sd * 100)`%) standard deviations, respectively (Figure 2).
...
Only `r n_outlier_func_reporting` (`r sprintf("%.1f", moe_prop_outlier_func_reporting$mean * 100)`%, 95% CI: `r sprintf("%.1f", moe_prop_outlier_func_reporting$lower * 100)`-`r sprintf("%.1f", moe_prop_outlier_func_reporting$upper * 100)`%) articles reported details of the function they used to calculate their outlier limit.
...
We found that `r count_specified_interpolation` (`r sprintf("%.1f", moe_prop_articles_reporting_interpolation$mean * 100)`%, 95% CI: `r sprintf("%.1f", moe_prop_articles_reporting_interpolation$lower * 100)`-`r sprintf("%.1f", moe_prop_articles_reporting_interpolation$upper * 100)`) out of `r format(total_articles, big.mark = ',', scientific = FALSE)` specified their interpolation methodology.
...
We recorded that `r n_reporting_avg_methods` (`r sprintf("%.1f", moe_avg_reporting$mean * 100)`%, 95% CI: `r round(moe_avg_reporting$lower * 100, 1)`-`r sprintf("%.1f", moe_avg_reporting$upper * 100)`%) reported some details of their data averaging methods.
We found `r sprintf("%.f", prop_multiple_methods * 100)` articles reported more than one averaging method.
...
Despite a much higher percentage of papers describing at least some of their averaging methods, approximately 40% of studies examined in this review neglected to document their process.
...
The same study also recommended a 15-breath rolling average or, preferably, the low-pass digital filter, but we only documented these methods `r english::english(n_15_breath_roll) %>% as.character()` (`r sprintf("%.1f", prop_15_breath_roll * 100)` ±`r sprintf("%.1f", moe_15_br_rolling * 100)`%) and `r english::english(n_low_pass) %>% as.character()` (`r sprintf("%.1f", prop_low_pass * 100)` ±`r sprintf("%.1f", moe_low_pass * 100)`%) times, respectively.
...
Our outlier reporting rates of `r sprintf("%.f", moe_prop_articles_reporting$mean * 100)` was similar, while our interpolation reporting rate of `r sprintf("%.1f", moe_prop_articles_reporting_interpolation$mean * 100)`% was slightly higher than Nolte's 4.3%.
Our averaging reporting rate was also \~5% higher at `r sprintf("%.1f", moe_avg_reporting$mean * 100)`.
Although the specific percentages differ, our results are similar to previous works showing that time-bin averages are the most popular, with 30-second averages as the most common overall [@midgley2007; @nolte2023; @robergs2010].
Similar to these works, ours showa that most studies do not follow current reporting guidelines.
...
However, due to its scope, not every article received a detailed examination, meaning some data processing descriptions might have been missed due to the limitations of our RegExs, leading us to categorize these as "not described." Articles that referred to previous works or supplemental materials for their data processing techniques were also marked as "not described" for simplicity.
...
Also, it is possible that a few ineligible articles eluded our screening.
Lastly, we predict that some studies we included collected their data via mixing chambers as not all authors specify this detail [@nolte2023].
...
Table 2: Averaging methods by type (a) and subtype (b).

**Figure Captions**

Figure 1: Selection of sources of evidence flow diagram in accordance with with the PRISMA 2020 statement

Dashed lines point to articles that were removed.
Solid lines indicate the path of articles that remained in the analysis.
Details are provided in the main text.
"Unobtained" articles were those unavailable due to University library licensing limitations or faulty download links.
"Resolvable" files could be analyzed by RegExs and ML.
