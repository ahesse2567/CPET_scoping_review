...
These methods [@tricco2018] and results [@peters2020] are modeled on the PRISMA scoping review extension guidelines. This work was first registered with the Open Science Framework [@foster2017; -@hesse2022] and is based on a dissertation chapter by the first author [-@hesse2023].
...
In contrast, we analyzed a random subset of articles using a random number generator in R to document data averaging methods because far more articles described their averaging methods.
...
Counts, percentages, and margin of error (95% confidence) were calculated for the reporting frequency of each data processing method using R `r str_extract(R.version.string, "R version \\d\\.\\d\\.\\d")` [@rcoreteam2021] in the RStudio IDE version `r rstudioapi::versionInfo()$version` [@rcoreteam2021].
...
Research by Martin-Rincon et al. \[@martin-rincon2019\] offers a strategy to compare two VO~2~max values obtained with different averaging methods. Without such corrections, one could misclassify cardiorespiratory fitness based on VO~2~max if VO~2~max were calculated with a sufficiently different sampling interval than that used to generate the normative data. However, this correction applies to group data \[@martin-rincon2019\] and different averaging strategies also affect individual VO~2~max values. In addition, the effect of different data averaging on VO~2~max is not uniform across individuals:

COMPUTE SUB ANALYSIS WITH PHYSIONET DATA TO SHOW HOW THIS INTERTWINES WITH ACSM AND FRIEND REGISTRY VO2MAX PERCENTILE CLASSIFICATION
...
Figure 2: Selection of sources of evidence flowchart.
...
Figure 3: Counts and percentages of outlier limits when specified.

Figure 4: Most prevalent specified interpolation methods by both type and time.

Figure 5: Prevalence of complete averaging procedures.
