...
This study used a single screening process, requring only BBB gas exchange data collection with exercise.
...
Counts, proportions, and Agresti-Coull 95% confidence intervals were calculated for the reporting frequency of each data processing method using R `r str_extract(R.version.string, "R version \\d\\.\\d\\.\\d")` [@rcoreteam2021] in the RStudio IDE version 2023.6.1.524 [@rcoreteam2021].
...
The initial search identified `r format(nrow(all_articles), big.mark = ',', scientific = FALSE)` articles; `format(nrow(articles_unique), big.mark = ',', scientific = FALSE)` remained for retrieval after removing duplicates and those without DOIs.
...
The most prevalent reported methods were ±3 (`r sprintf("%.1f", prop_3sd * 100)`%) and ±4 (`r sprintf("%.1f", prop_4sd * 100)`%) SD (Figure 2).
...
Of `r format(total_articles, big.mark = ',', scientific = FALSE)` articles `r count_specified_interpolation` (`r sprintf("%.1f", moe_prop_articles_reporting_interpolation$mean * 100)`%, 95% CI: `r sprintf("%.1f", moe_prop_articles_reporting_interpolation$lower * 100)`-`r sprintf("%.1f", moe_prop_articles_reporting_interpolation$upper * 100)` specified interpolation, with `r as.numeric(most_popular_interpolation_time) %>% english::english() %>% as.character()`-second intervals as the most common (`r sprintf("%.1f", prop_most_popular_interpolation_time * 100)`%).
Around half of reported interpolation procedures included the method (n = `r n_most_popular_interpolation_method`, `r sprintf("%.1f", prop_most_popular_interpolation_method * 100)`%), with `r most_popular_stated_interpolation_method` interpolation as the most popular (n = `r n_most_popular_stated_interpolation_method`, `r sprintf("%.1f", prop_most_popular_stated_interpolation_method * 100)`%) (Figure 3).
...
We recorded `r n_reporting_avg_methods` (`r sprintf("%.1f", moe_avg_reporting$mean * 100)`%, 95% CI: `r round(moe_avg_reporting$lower * 100, 1)`-`r sprintf("%.1f", moe_avg_reporting$upper * 100)`%) articles with averaging methods.
Of these, `r sprintf("%.f", prop_multiple_methods * 100)` articles reported more than one averaging method.
...
The same study also recommended a 15-breath rolling average or, preferably, the low-pass digital filter, but we only documented these methods `r english::english(n_15_breath_roll) %>% as.character()` (`r sprintf("%.1f", prop_15_breath_roll * 100)` and `r english::english(n_low_pass) %>% as.character()` (`r sprintf("%.1f", prop_low_pass * 100)`%) times, respectively.
