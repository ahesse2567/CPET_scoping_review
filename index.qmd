...
We are unaware of research on data processing and locating ventilatory thresholds.
...
Interpolation, often to one-second intervals, is common in VO2 kinetics research to "ensemble" average repeated transitions to minimize variability [@keir2014; @lamarra1987].
...
This may hamper reproduction or replication attempts, which became a more prominent issue in science within the past decade [@goodman2016; @opensciencecollaboration2015] Therefore, to assist with conducting future research on the effects of data processing on CPET values and to evaluate the methodological reproducibility of research using BBB gas exchange data generally, we conducted a broad scoping review to identify the prevalence and popularity of outlier removal, interpolation, and data averaging methods.
...
# don't ask why I need this chromote stuff bc I don't know
# https://github.com/rstudio/chromote/issues/5
# https://github.com/sneumann/metaRbolomics/blob/master/connectivity/revDepNet.Rmd
...
@fig-selection_sources_evidence_flowchart shows the selection of sources of evidence flowchart.
"Unobtained" articles were those unavailable due to University library licensing limitations or faulty download links.
We did not use the International Library Loan system given our already large sample.
...
#| fig-cap: Selection of sources of evidence flowchart. Dashed lines point to articles that were removed. Solid lines indicate the path of articles that remained in analysis. Details are provided in the main text.
#| fig-pos: 't'
...
`r str_to_sentence(most_popular_avg_type)` averages dominated in popularity (`r sprintf("%.1f", most_popular_avg_type_prop * 100)`%) (@tbl-avg_type_subtype_tables).
`r most_popular_avg_subtype` averages proved the most widespread averaging subtype (`r sprintf("%.1f", most_popular_avg_subtype_prop * 100)`%) (@tbl-avg_type_subtype_tables).
...
When incorporating averaging amounts, 30-, 60-, 15-, and 10-second bin averages (@fig-avg_full_method_plot) were the most popular.
...
Cubic spline interpolation produces a smooth curve but may slightly "overshoot" [@zhang1997] measured values.
Though likely small, we recommend authors specify the interpolation type.

Despite a much higher percentage of papers describing at least some of their averaging methods, a third of studies neglect to document their process.
...
Finally, we suspect a few ineligible articles eluded our screening.
Taken together, our results are not entirely comprehensive and slightly underestimate data processing methods' true reporting frequency.
...
This scoping review found that data processing methods were seldom reported for outlier removal and interpolation, and that averaging reporting, though much higher, could further improve. The results reflect prevalent methods, and prevalence should not be conflated with “best.” Nevertheless, knowing the prevalent methods can allow others to test the influence data processing in this field by comparing relevant options. Finally, we hope these results motivate others to improve their methodological documentation and, thus, reproducibility in this field.
