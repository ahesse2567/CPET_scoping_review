...
See the supplemental methods for additional ML model detials.
...
`r str_to_sentence(most_popular_avg_type)` averages dominated in popularity (`r sprintf("%.1f", most_popular_avg_type_prop * 100)`%) (Table 1).
`r most_popular_avg_subtype` averages proved the most widespread averaging subtype (`r sprintf("%.1f", most_popular_avg_subtype_prop * 100)`%) (Table 1).
...
We realize authors must balance adequate methodological documentation with word limits.
...
Several open-source gas data software are avaiable to facilitate transparent analyses [@maturana2021; @nolte2023; @hesse2023].
...
We predict that some studies we included collected their data via mixing chambers as not all authors specify this detail [@nolte2023].
...
Table 1: Averaging methods by type (a) and subtype (b).
