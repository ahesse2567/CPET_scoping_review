...
from helper_funcs.regex import non_OPRR_re, methods_res_disc
...
merge_df['is_OP-RR'].value_counts()
...
test_df['pred'] = test_df['text'].progress_apply(lambda x: 'e' if methods_res_disc(x) else 'i')
test_df['pred'].value_counts()
pred_df = test_df.drop('text', axis=1)[test_df['pred'] == 'i'].drop_duplicates().reset_index(drop=True)
pred_df.to_clipboard(index=False)
