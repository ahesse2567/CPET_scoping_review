...
# log = []
csv_path = '/Users/antonhesse/Desktop/Anton/Education/UMN/Lab and Research/HSPL/CPET_scoping_review/data/cpet_articles/unpaywall/springer_status_codes.csv'
...
for i, row in tqdm(articles.iterrows(), total=articles.shape[0]):
    ...
    try:
        ...
        out.update({'error': e})
    # try appending this information rather than relying on the log finishing properly
    pd.DataFrame([out]).to_csv(csv_path, mode='a', index=False, header=False)
    # log.append(out)

status_codes_df = pd.read_csv(csv_path).drop_duplicates('doi', keep='last').reset_index(drop=True)
status_codes_df.to_csv('csv_path')
