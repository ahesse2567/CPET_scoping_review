...
all_articles = pd.read_csv(str(Path('data/cpet_articles/unpaywall/unpaywall_info.csv')), dtype='str')
all_articles['doi_suffix'] = all_articles['doi'].apply(lambda x: get_doi_suffix(x))

merge_df = pd.merge(current_full_texts_df, all_articles, how='inner', on='doi_suffix').drop_duplicates(subset='doi_suffix')
