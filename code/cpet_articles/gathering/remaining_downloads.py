...
txt_file_paths = list(Path('data/cpet_articles/full_texts/txts').rglob('*/*.txt'))
txts = [path.stem for path in txt_file_paths]

epub_file_paths = list(Path('data/cpet_articles/full_texts/epubs').rglob('*/*.epub'))
...
remaining_articles['publisher'].value_counts()[0:10]